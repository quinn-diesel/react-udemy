...
44 - Action and Actions Creations
- creating a dynamic state is action and action creations
- lifecycle of an action
- start with:
- event triggered by a user. or could be an ajax request finishing loading up
- action creation: function that returns an action
- whenever there is an action it is called the action creator
- returns and object
- actions will flow through the reducer
- reducers can choose to return a different peice of state
- this is then piped into the application state
- this is then rendered back onto the page
- there is a type that is called for the action
- switch statement will go to a different line to the action
- if the type is matched it will return what you need

45 - Binding action creators
- need to make sure that the action creator will be properly wired up to redux and the back-end
- dispatch spits everything back out to the reducers
- this.props.selectbook is now what we are using

46 - Creating an action
- we have now made the object 
- want to call the action creator for what was called
- now there is the wiring to pipe all the information together

47 - reducer
- reducers always have two arguments when you are making them
- the state that gets passed is the state that is previously made by the reducer
- the state is flowing back into the reducer as actions are occuring
- anytime an action is triggered the state is incremented by one
- we need a base case to pass the state back through
- have to handle when the app boots up and the person doesn't have to learn
- we must always return a non-undefined value
- state = null, es6, if the action is null return it as null
- we never want to mutate what we are doing
- any key added to the reducer is added the global keys in the props

48 - Consuming action in reducer
- remember a container is something that has to touch the redux back-end
- the only purpose of app is to render to the booklist

49 - Conditonal Rendering
- since the reducer is getting called when the app starts up then it goes on
- we defaulted the state to null, our active book will be null
- our component is going to render and it is going to be null
- this will throw an error in javascript

50 - Review
- Redux is in change of managing the application state
- the application state is different to the component state
- component state is completely separate from the application state
- sometimes it can be easily done and easily connected
